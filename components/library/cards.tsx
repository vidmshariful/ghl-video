...
/* leaf imports, not lib/site: this card ships in the portal bundle too, and
 * the whole content file is a lot to carry for four helpers */
import { checkoutHref, withPromo } from "@/lib/content/premade";
import { nicheAddon, nicheAddonPrice, nicheIntro } from "@/lib/content/niche";
import { codeFor, skuFor } from "@/lib/content/codes";
import { cta } from "@/lib/content/core";
...
export function PackTile({
  count,
  bordered = true,
  label = "feature animations",
}: {
  count: number;
  bordered?: boolean;
  /* what the number counts. The tile was built for feature animation packs
     and said so in its own caption; on a bundle of 46 videos that caption
     was a lie printed in the brand gradient. */
  label?: string;
}) {
  return (
  ...
  return (
    <div className="group/card flex h-full flex-col border border-hair bg-card shadow-[0_4px_18px_-8px_rgba(0,0,0,0.6)] transition-all duration-300 hover:-translate-y-0.5 hover:border-gold/30 hover:shadow-[0_18px_40px_-14px_rgba(0,0,0,0.85)]">
      {video.wistiaId && video.poster ? (
        ...
        <PackTile
          count={video.packCount}
          bordered={false}
          label={video.typeTag === "Feature Animation" ? "feature animations" : "videos inside"}
        />
      ) : (
