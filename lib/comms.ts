...
export const COMM_ACTIONS: CommAction[] = [
  ...
  {
    key: "quote_sent", group: "custom", label: "A quote is sent",
    when: "A person sends a quote from the Custom screen. The client accepts or declines it on the quote page or in their portal.",
    mode: "manual",
    emails: [{ key: "quote_sent", to: "client" }],
    notifications: [],
  },
  {
    key: "quote_accepted", group: "custom", label: "The client accepts a quote",
    when: "The client accepts on our page. The project is made or priced at the agreed amount, the enquiry is marked won, and HighLevel's deal card follows.",
    mode: "automatic",
    emails: [{ key: "admin_quote_accepted", to: "team" }],
    notifications: [],
  },
  {
    key: "agreement_ready", group: "custom", label: "The retainer agreement is ready to accept",
    when: "A person presses Send the agreement on the client record. The partner accepts the terms in their portal with a typed name.",
    mode: "manual",
    emails: [{ key: "agreement_ready", to: "client" }],
    notifications: [],
  },
  {
    key: "review_request", group: "premade", label: "The review request",
    when: "The morning sweep, two days after a client's first delivered order or closed project. Once per client, and again only after six months.",
    mode: "scheduled",
    emails: [{ key: "review_request", to: "client" }],
    notifications: [],
  },
  {
