...
(async () => {
  ...
  console.log(
    "  2. Automation > a workflow on \"Contact Changed\" with a Webhook action to POST https://<site>/api/webhooks/highlevel/ carrying a custom header x-ghlv-key: <HIGHLEVEL_WEBHOOK_SECRET> (the header, never the URL), so edits made in HighLevel come back.",
  );
